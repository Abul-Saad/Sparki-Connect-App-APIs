...
const addWholesaler = async (req, res) => {
    ...
    // Basic validation
    if (!store_name || !current_status || !open_time || !close_time || !distance || !duration || !email || !mobile_number || !web_link || !latitude || !longitude) {
    ...
    const existingWholesaler = await userQuery(
        "SELECT * FROM wholesalers WHERE store_name = ? AND location = ? AND latitude = ? AND longitude = ? AND email = ? AND mobile_number = ?",
        [store_name, location, latitude, longitude, email, mobile_number]
    );
...
const getAllWholesalersAdmin  = async (req, res)=> {

    if(!req.user || req.user.userType !== "admin") {
        return res.status(403).json({
            status : "error",
            message : "Access denied. Only admins can get all wholsalers",
            statusCode : 403,
        });
    }
    ...
    try {
        const totalResult = await userQuery(`SELECT COUNT(*) AS total FROM wholesalers`);
        ...
        const wholesalers = await userQuery(query, [limit, offset]);

        return res.status(200).json({
            status : "success",
            message : "All wholesalers retrieved successfully",
            data : wholesalers,
            pagination : {
                total,
                totalPages,
...
const updateWholesaler = async (req, res) => {
    ...
    if (mobile_number !== undefined) fieldsToUpdate.mobile_number = mobile_number;
    if (web_link !== undefined) fieldsToUpdate.web_link = web_link;
